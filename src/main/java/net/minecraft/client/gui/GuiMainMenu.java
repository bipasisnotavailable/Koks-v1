...
import Bipas.Koks;
import Bipas.account.Account;
import Bipas.theme.ThemeSelection;
        ...
        if (button.id == 14 && this.realmsButton.visible) {
            try {
                ...
                if (clipboard.startsWith("api-"))
                    ACCOUNT.generateAltening(clipboard);
                else if(clipboard.contains(":") && split.length == 2)
                    ACCOUNT.login(split[0], split[1]);
                else
                    ACCOUNT.loginCracked(clipboard);
